...
信道扫描独立于客户端监控。页面默认折叠该区域，只有用户点击“扫描信道”时才调用 `/api/channel-scan`，后端执行 `iw dev <interface> scan` 并解析附近 BSS 的 SSID、BSSID、频率、信号和信道。扫描按钮从点击开始冷却 5 秒。前端将 2.4GHz、5GHz 上下分开画成以信号强度为高度的梯形占用图；当前网卡不支持 5GHz 时隐藏 5GHz 图表。扫描用于辅助人工选信道，不自动刷新、不缓存到配置，也不会改变热点当前使用的信道。某些驱动无法在 AP 模式下扫描，或受地区监管/DFS 限制导致扫描失败，页面会显示具体错误。
