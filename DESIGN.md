...
信道扫描独立于客户端监控。页面默认折叠该区域，只有用户点击“扫描信道”时才调用 `/api/channel-scan`，后端执行 `iw dev <interface> scan` 并解析附近 BSS 的 SSID、BSSID、频率、信号和信道。扫描按钮从点击开始冷却 5 秒。前端按选项卡分别展示 2.4GHz、5GHz 的信道占用图和热点表格；当前网卡不支持 5GHz 时隐藏 5GHz 选项卡。扫描用于辅助人工选信道，不自动刷新、不缓存到配置，也不会改变热点当前使用的信道。若网卡处于 `DOWN`，程序会仅为本次扫描临时置为 `UP`，结束后恢复原状态；若该网卡正在运行本程序创建的热点，程序会直接提示停止热点或使用另一块无线网卡，因为多数驱动不支持在同一接口的 AP 模式下扫描。地区监管、DFS 和驱动限制仍可能导致其他扫描失败，页面会显示具体错误。
